...
<script type="text/javascript">
    ...
    function one(pid, ispage) {
        ...
        var lprice = $("#lprice").val();
        var hprice = $("#hprice").val();
        var str = "?pid="+pid +"&ispage="+ispage+"&pname="+pname+"&typeid="+typeid+"&lprice="+lprice+"hprice="+hprice;
        location.href = "${pageContext.request.contextPath}/prod/one.action?pid=" + str;
    }
...
<!--分页的AJAX实现-->
    <script type="text/javascript">
        function  ajaxsplit(page) {
            ...
            var lprice = $("#lprice").val();
            var hprice = $("#hprice").val();
        //向服务端发送Ajax请求，请求page页中的所有数据，在当前页面上局部刷新
        $.ajax({
            url:"#{pageContext.request.contextPath}/prod/ajaxSplit.action",
